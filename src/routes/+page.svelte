<script lang="ts">
  ...
  const previewImageAction: Action = (node) => {
    ...
    const mouseOverHandler = (e: MouseEvent) => {
      ...
      const roationX = -4 * ((yVal - height / 2) / height);
      const rotationY = 4 * ((xVal - width / 2) / width);

      const animationString = `rotateX(${roationX}deg) rotateY(${rotationY}deg) scale(1.02)`;
...
<main>
  ...
  >
    ...
    >
      <div class="max-w-xl mb-5">
        ...
        <div class="p-3 flex flex-wrap gap-3 items-center max-w-xs">
          ...
          <a
            href="/auth?t=signin"
            class="btn variant-outline-secondary flex-1 font-semibold"
            >Sign-in</a
          >
        </div>
      </div>
    ...
    <section class="max-w-[80vw]">
      <div
        use:previewImageAction
        class="p-2 rounded-xl bg-surface-500/30 backdrop-blur-md shadow-md hover:shadow-xl"
      >
        <enhanced:img
  ...
  <section class="grid md:grid-cols-2 gap-4 max-w-[80vw] mx-auto my-4 md:p-10">
    <div class="flex flex-col gap-4 items-start">
      ...
      <p>
        Flashcards help you memorise everything you need to know. Make your
        learning easier and shorter.
      </p>
    ...
    <enhanced:img
      src="../lib/assets/cardPreview.jpeg"
      alt="Cards preview"
      class="rounded-xl"
    />
  </section>
...
<footer class="bg-primary-600 rounded-t-xl p-3 flex justify-between shadow-md">
  ...
  <p>
    <a href="https://github.com/zafchiel">@zafchiel</a>, {new Date().getFullYear()}
  </p>
</footer>
